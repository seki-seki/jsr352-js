 ...
 */
function getPropertyValues(parent, parentLabel) {
    var properties = parent && getPropertiesElement(parent, parentLabel);
 ...
 */
function getPropertiesElement(element, parentLabel) {
    if (!isExtensionElements(element)) {
        return element.properties;
    } else {
        return getPropertiesElementInsideExtensionElements(element, parentLabel);
    }
}
 ...
 * @return {ModdleElement} a camunda:Properties object
 */
function getPropertiesElementInsideExtensionElements(extensionElements, parentLabel) {
    return find(extensionElements.values, function (elem) {
        return is(elem, parentLabel);
    });
}
...
module.exports = function (element, bpmnFactory, options) {
    ...
    assign(options, {
        ...
        getElements: function (element, node) {
            ...
            return getPropertyValues(parent, parentLabel);
        },
        addElement: function (element, node) {
            ...
            if (!properties) {
                properties = elementHelper.createElement(parentLabel, {}, parent, bpmnFactory);
        ...
        updateElement: function (element, value, node, idx) {
            var parent = getParent(element, node, bo),
                    property = getPropertyValues(parent, parentLabel)[idx];
        ...
        validate: function (element, value, node, idx) {
            ...
            if (modelProperties.indexOf('id') >= 0) {
                ...
                var parent = getParent(element, node, bo),
                        properties = getPropertyValues(parent, parentLabel),
        ...
        removeElement: function (element, node, idx) {
            var commands = [],
                    ...
                    propertyValues = getPropertyValues(parent, parentLabel),
